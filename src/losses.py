...
def soft_skeletonize(x, thresh_width=2):
...
def norm_intersection(center_line, vessel):
    ...
    '''
    smooth = 1.
    clf = center_line.view(*center_line.shape[:1], -1)
    vf = vessel.view(*vessel.shape[:1], -1)
    intersection = (clf * vf).sum(-1)
    return (intersection + smooth) / (clf.sum(-1) + smooth)


def soft_cldice_loss(outputs, targets):  # bs, h, w / bs, h, w / 2, 512, 512

    cl_outputs = soft_skeletonize(outputs)
    cl_targets = soft_skeletonize(targets)

    iflat = norm_intersection(cl_outputs, targets)
    tflat = norm_intersection(cl_targets, outputs)

    intersection = iflat * tflat
    loss = (1 - (2 * intersection + eps) / (iflat + tflat)).mean()
...
class clDiceLoss(nn.Module):
    ...
    def forward(self, input, target):
        return soft_cldice_loss(input, target)
...
class ComboLoss(nn.Module):
    def __init__(
        ...
        self.expect_sigmoid = {"dice", "cldice", "focal", "jaccard", "lovasz_sigmoid"}
        self.per_channel = {"dice", "cldice", "jaccard", "lovasz_sigmoid"}
        self.values = {}
...
def mean(lov, ignore_nan=False, empty=0):
    """
    nanmean compatible with generators.
    """
    lov = iter(lov)
    if ignore_nan:
        lov = filterfalse(np.isnan, lov)
    try:
        n = 1
        acc = next(lov)
